(ns sawmilling.governor
  "Sawmill Plant Operations Governor -- the independent compliance
  layer that earns the SawmillAdvisor the right to commit. The advisor
  ...
  tries to FINALIZE (rather than merely draft-schedule) a kiln
  schedule, whether a shipment proposal's own claimed volume would
  blow through the batch's own logged production volume, or when an
  act stops being a coordination proposal and becomes direct saw/
  planer/kiln-equipment control, so this MUST be a separate system
  able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:sawmill-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       `:kiln/finalize`) is the 'direct
                                       saw/planer/kiln-equipment
                                       control' scope violation this
                                       actor must NEVER perform --
                                       HARD, PERMANENT, unconditional.
    4. Kiln-finalize blocked       -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:finalize? true`?
                                       Finalizing a kiln schedule is
    ...
    8. Shipment volume exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       `:shipped-volume-board-ft` plus
                                       the proposal's own claimed
                                       `:volume-board-ft` would exceed
                                       the batch's own recorded
                                       `:volume-board-ft`
                                       (`sawmilling.registry/shipment-
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [sawmilling.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct saw/planer/
  kiln-equipment-control or kiln-schedule-finalize effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct saw/planer/kiln-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は鋸盤・プレーナー・キルン設備の直接操作/キルンスケジュール確定に該当する可能性があり、恒久的に禁止")}]))

(defn- kiln-finalize-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:finalize? true` is attempting to
  finalize a kiln schedule directly -- this actor may only ever
  propose/schedule a DRAFT maintenance window, never finalize/execute
  a kiln run. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:finalize? (:value proposal))))
    [{:rule :kiln-finalize-blocked
      :detail "キルンスケジュールの確定(finalize)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- shipment-volume-exceeded-violations
  ...
  `:volume-board-ft` -- ground truth from the batch's own permanent
  fields, never a self-reported volume claim."
  [{:keys [op]} proposal st]
  (when (= op :coordinate-shipment)
    (let [{:keys [batch-id volume-board-ft]} (:value proposal)
          b (and batch-id (store/batch st batch-id))]
      (cond
        ;; No batch, no recorded capacity, or no stated volume: the
        ;; headroom cannot be computed, so it is not headroom. This used
        ;; to fall through as "not over capacity" and ship.
        (not (registry/shipment-volume-checkable? b volume-board-ft))
        [{:rule :shipment-volume-exceeded
          :detail (str batch-id " の生産量/既存出荷実績/申請量のいずれかが数値として確定できない -- "
                       "空き容量を検算できないため出荷しない")}]

        (registry/shipment-volume-exceeded? b volume-board-ft)
        [{:rule :shipment-volume-exceeded
          :detail (str batch-id " の記録済み生産量(" (:volume-board-ft b)
                       "board-ft)を、既存出荷実績(" (:shipped-volume-board-ft b 0.0)
                       "board-ft)+今回申請(" volume-board-ft "board-ft)が超過")}]))))
...
(defn- invalid-moisture-content-violations
  ...
  `:moisture-content-percent` that is not a physically plausible
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [mc (:moisture-content-percent (:value proposal))]
      (when (and (some? mc) (not (registry/moisture-content-valid? mc)))
        [{:rule :invalid-moisture-content
          :detail (str mc "% は物理的に妥当な含水率の範囲外")}]))))

(defn check
  "Censors a SawmillAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (kiln-finalize-blocked-violations request proposal)
