...
(defn- exec! [actor tid request] (g/run* actor {:request request :context coordinator} {:thread-id tid}))
(defn- approve! [actor tid] (g/run* actor {:approval {:status :approved :by "coord-1"}} {:thread-id tid :resume? true}))

(defn run-demo! []
  (let [db (-> (store/mem-store) (store/sample-data!))
        actor (op/build db)]
    (exec! actor "t1" {:op :log-production-batch :effect :propose :subject "batch-001"
                       :patch {:grade :no1 :last-assessed "2026-07-14"}})
    (exec! actor "t2" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                       :patch {:equipment "saw-blade-1" :window "2026-08-01"}})
    (approve! actor "t2")
    (exec! actor "t3" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                       :patch {:severity :moderate :description "blade-guard-misalignment"}})
    (approve! actor "t3")
    (exec! actor "t4" {:op :log-production-batch :effect :direct-write :subject "batch-002"
                       :patch {:grade :no2}})
    db))
...
(def ^:private gate-rows
  ["        <tr><td><code>:log-production-batch</code></td><td><span class=\"ok\">auto-commit when clean</span></td></tr>"
   "        <tr><td><code>:schedule-maintenance</code></td><td><span class=\"warn\">ALWAYS human approval</span></td></tr>"
...
(defn render [db]
  (let [ledger (vec (store/ledger db))
        ...
        brow (fn [b] (format "        <tr><td>%s</td><td>%s</td><td>%s</td></tr>" (esc (:id b)) (esc (str (or (:grade b) "-"))) (status-cell ledger (:id b))))
        brows (str/join "\n" (map brow batches))
        lrows (str/join "\n" (map ledger-row ledger))]
    (str "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-1610</title>"
     "<style>body{font:14px/1.5 sans-serif;margin:0;color:#1a1a1a;background:#f5f5f5}"
     ".bar{background:#1a2a0a;color:#fff;padding:1.2rem 2rem}.bar h1{margin:0;font-size:1.15rem}"
     ...
     "<header class=\"bar\"><h1>Sawmilling ops (ISIC 1610) — <code>sawmilling</code></h1></header><main>"
     "<section class=\"card\"><h2>Production batches</h2>"
     "<p class=\"muted\">Demo from <code>sawmilling.store</code> via <code>sawmilling.render-html</code>. No invented data.</p>"
     "<table><thead><tr><th>Batch</th><th>Grade</th><th>Last op</th></tr></thead><tbody>" brows "</tbody></table></section>"
