(ns sawmilling.store
  "SSoT for the sawmill plant-operations coordination actor, behind a
  ...
  Decision 1, this vertical is self-contained (no external sawmilling
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             lumber-grade/volume/moisture-content
                             ...
                             `:shipped-volume-board-ft` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a saw-blade/planer/kiln unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled saw-blade/planer/kiln
                             maintenance-window DRAFT against a piece
                             of equipment (`sawmilling.registry`'s
    ...
    - `shipments`         -- a proposed outbound-lumber-shipment DRAFT
                             (`sawmilling.registry`'s
                             `register-shipment`).
  ...
  audit trail a mill owner or lumber buyer trusting this coordinator
  needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (sawmilling.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (sawmilling.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :no2 :species "Douglas Fir"
                :volume-board-ft 50000.0 :moisture-content-percent 15.0
                :verified? true :registered? true
                :shipped-volume-board-ft 10000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :select-structural :species "Ponderosa Pine"
                :volume-board-ft 8000.0 :moisture-content-percent 12.0
                :verified? true :registered? true
                :shipped-volume-board-ft 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :stud :species "Western Hemlock"
                :volume-board-ft 60000.0 :moisture-content-percent 18.0
                :verified? false :registered? false
                :shipped-volume-board-ft 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :saw-blade
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :kiln
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `sawmilling.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `sawmilling.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-volume-board-ft]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:volume-board-ft value) 0.0))))))))
        result)
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered saw-blade unit
  (schedulable for maintenance), one UNVERIFIED/unregistered kiln unit
  (blocks any maintenance scheduling against it) -- so the actor + demo
