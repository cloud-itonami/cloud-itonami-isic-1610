(ns sawmilling.operation
  "SawmillOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (SawmillAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Sawmill Plant Operations
  ...
  (:require [langgraph.graph :as g]
            ...
            [sawmilling.advisor :as advisor]
            [sawmilling.governor :as governor]
            [sawmilling.phase :as phase]
...
(defn build
  "Compiles a SawmillOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `sawmilling.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SawmillAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Sawmill Plant Operations Governor -- independent censor.
      (g/add-node :govern
