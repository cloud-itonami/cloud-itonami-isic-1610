(ns sawmilling.advisor
  "SawmillAdvisor -- the *contained intelligence node* for the sawmill
  plant-operations coordination actor.

  It normalizes production-batch patches (lumber-grade/volume/
  moisture-content), drafts a saw-blade/planer/kiln maintenance
  scheduling proposal against a piece of equipment, drafts a
  safety-concern flag, and drafts an outbound-lumber-shipment
  ...
  a real saw/planer/kiln-equipment actuation, kiln-schedule execution,
  or freight dispatch. Every output is censored downstream by
  `sawmilling.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct saw/planer/
                                 ; kiln-equipment-control or
                                 ; kiln-schedule-finalize effect
  ...
  contract every caller of this actor agrees to) -- `sawmilling.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [sawmilling.registry :as registry]
...
(defn- log-production-batch
  ...
  moisture-content or verification status. High confidence, low
...
(defn- schedule-maintenance
  "Draft a saw-blade/planer/kiln maintenance-window scheduling
  ...
  `sawmilling.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft an equipment/kiln-fire/dust-hazard safety-concern flag.
  ...
  technicality). See `sawmilling.phase`: no phase ever adds this op to
  a phase's `:auto` set; `sawmilling.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound-lumber-shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed volume -- it does NOT invent one, and `sawmilling.governor`
  ...
  (let [batch-id (:batch-id value)
        ...
        over-volume? (and b (registry/shipment-volume-exceeded?
                             b (:volume-board-ft value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:volume-board-ft value) " board-ft)"
...
(def ^:private system-prompt
  (str "あなたは製材工場プラント運用コーディネーターの助言者です。"
       ...
       "キルスケジュールの確定(finalize)や鋸盤・プレーナー等の設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `sawmilling.governor`
