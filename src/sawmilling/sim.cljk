(ns sawmilling.sim
  ...
  production volume, a proposal that tries to FINALIZE a kiln schedule
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, saw-blade -- escalates, approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-saw-blade :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered kiln -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (1000 board-ft would exceed volume 8000 vs shipped 7500 -> HARD hold) ==")
    ...
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:moisture-content-percent 999.0}}
                       coordinator))
