(ns sawmilling.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control saw/planer/kiln equipment
  directly... does NOT authorize or execute maintenance/kiln
  operations') implemented faithfully. The single invariant under
  test:

    SawmillAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Sawmill Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:grade :no1}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :no1 (:grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-saw-blade :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "equip-002" :maintenance-type :kiln-inspection
                             :scheduled-date "2026-08-01" :finalize? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t6"
                    {:op :coordinate-shipment :effect :propose :subject "ship-2"
                     :value {:batch-id "batch-003" :volume-board-ft 1000.0
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t7"
                    {:op :coordinate-shipment :effect :propose :subject "ship-3"
                     :value {:batch-id "batch-002" :volume-board-ft 1000.0
...
(deftest kiln-finalize-is-held-and-permanently-blocked
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "equip-001" :maintenance-type :kiln-run-finalize
                             :scheduled-date "2026-09-01" :finalize? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:kiln-finalize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
...
(deftest invalid-moisture-content-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:moisture-content-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-moisture-content} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:moisture-content-percent (store/batch db "batch-001"))) "fabricated moisture-content never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "dust near saw blade"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
  ...
  (testing "a CLEAN shipment coordination is never auto-eligible -- always escalates, even below any volume threshold"
    (let [[db actor] (fresh)
          res (exec-op actor "t14" {:op :coordinate-shipment :effect :propose :subject "ship-1"
                                    :value {:batch-id "batch-001" :volume-board-ft 5000.0
